  ...
  <div class="h-full flex flex-col bg-white dark:bg-gray-900">
    ...
    <div class="flex-1 overflow-y-auto">
      ...
      <div v-else-if="!loading && filteredHistoryList.length === 0" class="p-8 text-center text-gray-500 dark:text-gray-400">
        ...
        <p v-if="selectedProduct" class="text-base font-medium">该产品暂无多轮优化记录</p>
        <p v-else class="text-base font-medium">还没有多轮优化记录</p>
        <p class="text-sm mt-1">使用多轮优化生成后会自动保存到这里</p>
      </div>
        ...
        >
          ...
          <div class="flex items-start justify-between mb-3">
            <div class="flex-1">
              <h3 class="text-sm font-medium text-gray-900 dark:text-white mb-1 line-clamp-2">
                {{ record.optimizedContent?.title }}
              </h3>
              <div class="flex items-center space-x-2 text-xs text-gray-500 dark:text-gray-400">
                <span class="flex items-center">
                  📦 {{ record.generationParams?.productName }}
                </span>
                <span>•</span>
                <span>🚀 多轮优化</span>
                <span>•</span>
          ...
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center space-x-4">
              <div class="text-xs">
                <span class="text-gray-600 dark:text-gray-400">综合评分:</span>
                <span class="text-purple-600 dark:text-purple-400 font-semibold ml-1">
                  {{ record.qualityAnalysis?.overallScore || 'N/A' }}
                </span>
              </div>
              <div class="flex space-x-2">
                <span class="inline-block w-2 h-2 bg-green-400 rounded-full" title="真实感"></span>
                <span class="inline-block w-2 h-2 bg-blue-400 rounded-full" title="爆款潜力"></span>
                <span class="inline-block w-2 h-2 bg-orange-400 rounded-full" title="平台适配"></span>
              </div>
            </div>
            <span class="text-xs text-gray-500 dark:text-gray-400">
              {{ record.metadata.total_generation_time }}ms
            </span>
          </div>

          <!-- 预览内容 -->
          <div class="text-xs text-gray-600 dark:text-gray-300 mb-3">
            <p class="line-clamp-2">{{ record.optimizedContent?.bodyPlain }}</p>
          </div>

          <!-- 多轮处理标签 -->
          <div class="flex flex-wrap gap-1">
            <span class="inline-block px-2 py-0.5 bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200 text-xs rounded">
            ...
            <span v-if="record.generationParams?.keywords" class="inline-block px-2 py-0.5 bg-orange-100 dark:bg-orange-900 text-orange-800 dark:text-orange-200 text-xs rounded">
...
const loadHistory = async (reset = false) => {
  try {
    ...
    const response = await getMultiRoundHistory(pagination.page, pagination.pageSize)
    ...
    console.log(`📋 多轮优化历史记录加载完成: 第${pagination.page}页, ${response.records.length}条`)
  } catch (error) {
    console.error('❌ 加载多轮优化历史记录失败:', error)
  } finally {
    loading.value = false
...
const loadStats = async () => {
  try {
    const response = await getMultiRoundHistoryStats()
    Object.assign(stats, response)
  } catch (error) {
    console.error('❌ 加载多轮优化统计信息失败:', error)
  }
}
...
const deleteRecord = async (id) => {
  if (!confirm('确定要删除这条多轮优化记录吗？')) {
    return
  }

  try {
    await deleteMultiRoundHistory(id)
...
const clearAllHistory = async () => {
  if (!confirm('确定要清空所有多轮优化历史记录吗？此操作不可恢复！')) {
    return
  }
  ...
  try {
    ...
    // 逐条删除（因为没有批量删除接口）
    const deletePromises = historyList.value.map(record => deleteMultiRoundHistory(record.id))
    ...
    console.log('✅ 多轮优化历史记录清空成功')
  } catch (error) {
    console.error('❌ 清空多轮优化历史记录失败:', error)
    alert('清空失败，请重试')
  } finally {
    loading.value = false
  }
